...
- [ ] T114 [US1] Reflow generated comments to the print width, in the format step rather than in each template. Prettier wraps code and never comments, so prose written at this repo's own width ships unwrapped into files formatted at 80: measured on the current three patterns, 34 comment lines in `retry`, 13 in `circuit-breaker`, and 2 in `result` ran between 81 and 172 columns. The evidence that this cannot be fixed template-by-template: a pattern composes `dedent` blocks inside other `dedent` blocks, so the column a comment will finally sit at is not knowable where the comment is written, and wrapping against the wrong column is what produced the 82-column lines that `docAt` was added to fix. What is already done — `sections`, `doc`, `docAt`, and `wrapProse` in `src/engine/render/helpers.ts`, plus a mechanical reflow of the literal comment runs in all three patterns — removed the worst of it and is worth keeping regardless, but it cannot close the gap. The fix is a post-format pass over comment token ranges (via the compiler's scanner, so nothing inside a string or template literal is ever touched), which fixes every pattern at once including future ones. `overWideComments` in `test/golden/harness.ts` already computes the violation; landing this makes the golden suite's assertion one line.
