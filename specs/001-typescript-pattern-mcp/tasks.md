...
- [ ] T112 [US1] **Needs a decision, because the premise does not hold.** data-model.md §"Shared base options" says all seven are "present on every generative pattern", and authoring a second pattern has now shown that three of them cannot be. `cancellation` is settled — `retry` declares it and honours it (T035). The rest:
  ...
  So the honest shape is a *conditional* base set: `errorMode`/`async`/`cancellation`/`includeTests` where they are meaningful, `emitScope`/`coreModule` iff `supportsSplit`, `verbosity` never. That is a change to data-model.md, not just to the catalog, which is why this is not being done unilaterally. Meanwhile `src/mcp/tools/generate.ts` continues to omit fields no pattern declares rather than advertising inputs that can only be refused.
