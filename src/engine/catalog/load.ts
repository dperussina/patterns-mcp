...
import { CatalogShardSchema, type Pattern } from "./schema.js";
...
const DEFAULT_DIRECTORY = fileURLToPath(new URL("../../../data/patterns/", import.meta.url));
...
export function buildCatalog(shards: readonly ShardSource[]): Catalog {
  ...
  for (const [name, pattern] of [...byName].toSorted(([a], [b]) => compareNames(a, b))) {
  ...
  const patterns = [...byName.values()].toSorted((a, b) => compareNames(a.name, b.name));
 ...
 */
export async function loadCatalog(directory: string = DEFAULT_DIRECTORY): Promise<Catalog> {
  const entries = await readdir(directory);
  const shardFiles = entries.filter((entry) => entry.endsWith(".json")).toSorted(compareNames);

  const shards: ShardSource[] = [];
  for (const file of shardFiles) {
    const raw = await readFile(new URL(file, pathToDirectoryUrl(directory)), "utf8");
    shards.push({ source: file, contents: parseJson(file, raw) });
  }

  return buildCatalog(shards);
}
